...
setwd("U:/UF/Extension specific/CFEE/Effort trends/code/Markdown/2025")  #set working directory to where any input files exist
	countlist <- read.csv("countlist_parms_eff2pp.csv", header=T, colClasses=c("character")) #this reads in a csv of parameters for each county. It reads them all in as characters
	...
	tryCatch({
		tmplist <- as.list(countlist[i,])   #this is the what does it--the "templist" creates a list object with $ operators for each row (county) in the input file. This means tmplist$county returns "Brevard"
	  	rmarkdown::render( 					#this calls render from markdown
	    	'county_effort_2pp.Rmd', output_file = paste0('2025_Effort_', tmplist$county, '_2pp.docx') #this first item is the input file--you need a "general" .Rmd files with any parameters called as such--so if you want the ouput to say "Brevard", the input should say `r templist$county`
		)
...
#   rmarkdown::render( 					#this calls render from markdown
#     'county_effort_2pp.Rmd', output_file = paste0('2022_Effort_', tmplist$county, '_2ppTEST.docx') #this first item is the input file--you need a "general" .Rmd files with any parameters called as such--so if you want the ouput to say "Brevard", the input should say `r templist$county`
# 	)   
