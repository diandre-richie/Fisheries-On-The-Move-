...
library(dplyr)
	...
	# Create a list of the files you want to read in
		setwd("U:/UF/Data/NMFS/MRIP data/MRIP/CSVs/raw/1990_1994/trips") #for whatever reason, it seems that you must re-direct the WD...not sure why
		all.files <- list.files(path = "U:/UF/Data/NMFS/MRIP data/MRIP/CSVs/raw/1990_1994/trips", pattern = ".csv") #this is a test trips from 2018--watch for cap changes with other years
	
	#This is if you are want ALL the columns
		l <- lapply(all.files, fread, sep=",") #read them all in using fread

			# for(i in 1:length(l)) {					#changing all the names to lowercase so that you avoid potential conflicts
			# 	names(l[[i]]) <- tolower(names(l[[i]]))
			# 	}

			# for(i in 1:length(l)) {					#changing all the names to lowercase so that you avoid potential conflicts
			# 	l[[i]] <- l[[i]][,list(prim2_common, prim1_common, strat_id, psu_id, year, reg_res, st_res, cnty_res, st, 
			...
			# for(i in 1:length(l)) {					#changing all the names to lowercase so that you avoid potential conflicts
			# 	l[[i]] <- l[[i]][, id_code:=as.character(id_code)]
			# 	}
			...
			for(i in 1:length(l)) {					
				...
				l[[i]] <- l[[i]][,list(prim2_common, prim1_common, strat_id, psu_id, year, reg_res, st_res, cnty_res, st, #subsetting the columns you care about
					cnty, intsite , mode_f, mode_fx, area, area_x, hrsf, add_hrs, id_code, sub_reg, wave, catch, 
					boat_hrs, month, kod, county, time, wp_int, var_id, leader)]
				l[[i]] <- l[[i]][, psu_id:=as.character(psu_id)]		#coercing some columns to character
	...
	#Writing out data
		...
		save(dt, file = "All_1990_1994.RData")
		save(dt_fl, file = "FL_1990_1994.RData")
